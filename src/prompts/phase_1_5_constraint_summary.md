...
Generate a migration constraint summary for a CUDA-to-NPU migration project.
...
2. For each user constraint, derive 1-2 specific, imperative migration rules. For example:
   - If user says "zero CPU fallback" → "Do not accept migrated execution paths that redirect accelerator work to CPU fallback."
...
4. Make each rule specific and testable — do NOT produce generic rules like "use NPU instead of CUDA".
